...
/// <summary>
/// Dispatcher interno para comandos sem retorno, responsável por validação e execução otimizada.
/// </summary>
internal static class VoidCommandDispatcher<TCommand> where TCommand : ICommand
{
    private static Func<IServiceProvider, TCommand, CancellationToken, ValueTask>? _invoker;
    ...
    public static async ValueTask Send(IServiceProvider sp, TCommand cmd, CancellationToken ct)
    {
        ...
        if (invoker != null)
        {
            await invoker(sp, cmd, ct).ConfigureAwait(false);
            return;
        }

        await DispatchSlow(sp, cmd, ct).ConfigureAwait(false);
    }
    ...
    /// <returns>Uma tarefa que representa a operação assíncrona.</returns>
    [MethodImpl(MethodImplOptions.NoInlining)]
    private static ValueTask DispatchSlow(IServiceProvider sp, TCommand cmd, CancellationToken ct)
    {
        var handler = sp.GetRequiredService<ICommandHandler<TCommand>>();

        static ValueTask newInvoker(IServiceProvider serviceProvider, TCommand command, CancellationToken cancellationToken)
        {
